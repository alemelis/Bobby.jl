@testset "magic.jl" begin
    ...
    [@test Bb.ROOK_BITS[Bb.sq2idx(p2u[s])] == 12 for s in ["a1", "a8", "h1", "h8"]]
    [@test Bb.ROOK_BITS[Bb.sq2idx(p2u[s])] == 11 for s in ["a4", "f8", "e1", "h2"]]
    [@test Bb.ROOK_BITS[Bb.sq2idx(p2u[s])] == 10 for s in ["b4", "d3", "f7", "g2"]]
    [@test Bb.BISHOP_BITS[Bb.sq2idx(p2u[s])] == 6 for s in ["a1", "a8", "h1", "h8"]]
    [@test Bb.BISHOP_BITS[Bb.sq2idx(p2u[s])] == 5 for s in ["a3", "g5", "e2", "f8"]]
    [@test Bb.BISHOP_BITS[Bb.sq2idx(p2u[s])] == 7 for s in ["c3", "f5", "e3"]]
    [@test Bb.BISHOP_BITS[Bb.sq2idx(p2u[s])] == 9 for s in ["d4", "e5"]]
