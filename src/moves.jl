...
function validate_move(chessboard::Chessboard, move::Move, player_color::String)
    ...
    # if ~check_unmove(chessboard, chessboard_)
    #     error("nononononononono")
    # end
    return ~in_check
end
...
function update_castling_rights(chessboard::Chessboard)
    if chessboard.white.K == E1
        chessboard.white_king_moved = false
    end
    if A1 == chessboard.white.R
        board.white_can_castle_queenside = true
    end
    if H1 == chessboard.white.R
        chessboard.white_can_castle_kingside = true
    end
    if chessboard.black.K == E8
        chessboard.black_king_moved = false
    end
    if A8 == chessboard.black.R
        chessboard.black_can_castle_queenside = true
    end
    if H8 == chessboard.black.R
        chessboard.black_can_castle_kingside = true
    end
    for move in chessboard.game
        if move == "whitekinge1"
            chessboard.white_king_moved = true
        elseif move == "whiterooka1"
            chessboard.white_can_castle_queenside = false
        elseif move == "whiterookh1"
            chessboard.white_can_castle_kingside = false
        elseif move == "blackkinge8"
            chessboard.black_king_moved = true
        elseif move == "blackrooka8"
            chessboard.black_can_castle_queenside = false
        elseif move == "blackrookh8"
            chessboard.black_can_castle_kingside = false
        end
    end
    return chessboard
end
