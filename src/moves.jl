...
function move_piece!(board::Bitboard, move::Move, color::String="white")
    if color == "white"
        update_from_to_squares!(board.white, move.source, move.target)
        update_from_to_squares!(board.taken, move.source, move.target)

        if move.capture_type != "none"
            remove_from_square!(board.black, move.target)
            if move.capture_type == "pawn"
                remove_from_square!(board.p, move.target)
            elseif move.capture_type == "night"
                remove_from_square!(board.n, move.target)
            elseif move.capture_type == "bishop"
                remove_from_square!(board.b, move.target)
            elseif move.capture_type == "queen"
                remove_from_square!(board.q, move.target)
            elseif move.capture_type == "rook"
                remove_from_square!(board.r, move.target)
            end
        end

        if move.piece_type == "pawn"
            # if move.target == board.enpassant_square
            #     remove_from_square!(board.black, move.target>>8)
            #     remove_from_square!(board.p, move.target>>8)
            #     remove_from_square!(board.taken, move.target>>8)
            #     board.enpassant_done = true
            # else
            #     board.enpassant_done = false
            # end
            update_from_to_squares!(board.P, move.source, move.target)
            # board.enpassant_square = move.enpassant_square
        elseif move.piece_type == "night"
            update_from_to_squares!(board.N, move.source, move.target)
        elseif move.piece_type == "bishop"
            update_from_to_squares!(board.B, move.source, move.target)
        elseif move.piece_type == "queen"
            update_from_to_squares!(board.Q, move.source, move.target)
        elseif move.piece_type == "rook"
            update_from_to_squares!(board.R, move.source, move.target)
        elseif move.piece_type == "king"
            update_from_to_squares!(board.K, move.source, move.target)
        end
        ...
        #         add_to_square!(board.Q, move.target)
        #     elseif move.promotion_type == "rook"
        #         add_to_square!(board.R, move.target)
        #     elseif move.promotion_type == "night"
        #         add_to_square!(board.N, move.target)
        #     elseif move.promotion_type == "bishop"
        #         add_to_square!(board.B, move.target)
        #     end
        # end
    else
        update_from_to_squares!(board.black, move.source, move.target)
        update_from_to_squares!(board.taken, move.source, move.target)

        if move.capture_type != "none"
            remove_from_square!(board.white, move.target)
            if move.capture_type == "pawn"
                remove_from_square!(board.P, move.target)
            elseif move.capture_type == "night"
                remove_from_square!(board.N, move.target)
        ...
        if move.piece_type == "pawn"
            # if move.target == board.enpassant_square
            #     remove_from_square!(board.white, move.target<<8)
            #     remove_from_square!(board.P, move.target<<8)
            #     remove_from_square!(board.taken, move.target<<8)
            #     board.enpassant_done = true
            # else
...
function unmove_piece!(board::Bitboard, move::Move, color::String="white")
    if color == "white"
        update_from_to_squares!(board.white, move.target, move.source)
        update_from_to_squares!(board.taken, move.target, move.source)

        if move.piece_type == "pawn"
            # if board.enpassant_done
            #     add_to_square!(board.black, move.target>>8)
            #     add_to_square!(board.p, move.target>>8)
            #     add_to_square!(board.taken, move.target>>8)
            #     board.enpassant_done = false
            #     board.enpassant_square = move.target
            # end
            update_from_to_squares!(board.P, move.target, move.source)
        elseif move.piece_type == "night"
            update_from_to_squares!(board.N, move.target, move.source)
        elseif move.piece_type == "bishop"
            update_from_to_squares!(board.B, move.target, move.source)
        elseif move.piece_type == "queen"
            update_from_to_squares!(board.Q, move.target, move.source)
        elseif move.piece_type == "rook"
            update_from_to_squares!(board.R, move.target, move.source)
        elseif move.piece_type == "king"
            update_from_to_squares!(board.K, move.target, move.source)
        end

        if move.capture_type != "none"
            add_to_square!(board.black, move.target)
            add_to_square!(board.taken, move.target)
            if move.capture_type == "pawn"
                add_to_square!(board.p, move.target)
            elseif move.capture_type == "night"
                add_to_square!(board.n, move.target)
            elseif move.capture_type == "bishop"
                add_to_square!(board.b, move.target)
        ...
        update_from_to_squares!(board.black, move.target, move.source)
        update_from_to_squares!(board.taken, move.target, move.source)

        if move.piece_type == "pawn"
            # if board.enpassant_done
            #     add_to_square!(board.white, move.target<<8)
            #     add_to_square!(board.P, move.target<<8)
            #     add_to_square!(board.taken, move.target<<8)
            #     board.enpassant_done = false
            #     board.enpassant_square = move.target
        ...
        if move.capture_type != "none"
            add_to_square!(board.white, move.target)
            add_to_square!(board.taken, move.target)
            if move.capture_type == "pawn"
                add_to_square!(board.P, move.target)
            elseif move.capture_type == "night"
